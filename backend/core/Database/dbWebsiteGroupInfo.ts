...
export async function getWebsiteGroupInfo(): Promise<WebsiteGroupInfo[]> {
	let result = await prisma.websiteGroupInfo.findMany();
	if (result.length === 0) {
...
export async function createWebsiteGroupInfo() {
	...
	await prisma.websiteGroupInfo.create({
		data: {
			...
			nextIdentifier:
				"nav.nav-offcanvas.hide-for-large-up > ul > li.active > ul > li.active > ul > li:nth-child({N}),div.large-9.large-push-3.columns > ul > li{ID#Selected masterpieces}",
		},
	});
}
